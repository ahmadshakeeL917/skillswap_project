...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.admin:
            st.markdown(f"**Admin:** {st.session_state.admin['username']}")
            st.markdown("---")
            pages = {
            ...
            if i + j < len(listings):
                ...
                with col:
                    with st.container(border=True):
                        ...
                        st.caption(f"{(l.get('users') or {}).get('fullname', 'Student')} | Rating: {(l.get('users') or {}).get('avgrating', 0):.1f}")
...
def page_jobs():
            ...
            with st.container(border=True):
                ...
                if u:
                    ...
                    if is_poster:
                        ...
                        if bids:
                            with st.expander(f"View {len(bids)} Bid(s)"):
                                for bid in bids:
    ...
    with tab2:
        ...
        if u["role"] not in ("buyer", "both"):
            st.warning("Only buyers can post jobs. Update your role!")
            return

        st.info("Your job will be reviewed by admin before going live.")
        categories = db_helper.get_categories()
                ...
                if o["escrowstatus"] == "active":
                    ...
                    with bc1:
                        if st.button("Mark Complete", key=f"comp_{o['orderid']}"):
                            ...
                            st.rerun()
                    with bc2:
                        if st.button("Raise Dispute", key=f"disp_{o['orderid']}"):
    ...
    with tab1:
        ...
        for l in listings:
            with st.container(border=True):
                ...
                with c2:
                    if st.button("Delete", key=f"del_{l['listing_id']}"):
...
def page_admin_dashboard():
    ...
    with c4: st.metric("Open Disputes", stats["open_disputes"])
            ...
            with c2:
                ...
                if l["approvalstatus"] != "rejected":
                    if st.button("Reject", key=f"rej_{l['listing_id']}", use_container_width=True):
                        ...
                        st.warning("Rejected!")
                        st.rerun()
                if st.button("Delete", key=f"del_{l['listing_id']}", use_container_width=True):
                    db_helper.delete_listing(l["listing_id"])
                    st.success("Deleted!")
                    st.rerun()
    ...
    for u in users:
        with st.container(border=True):
            ...
            with c1:
                ...
                st.caption(f"{u['walletbalance']} pts | Rating: {u.get('avgrating',0):.1f} | ID: {u['studentid']}")
            with c2:
                ...
                if st.button(label, key=f"sus_{u['studentid']}", use_container_width=True):
                    ...
                    st.rerun()
                if st.button("Delete", key=f"del_{u['studentid']}", use_container_width=True):
